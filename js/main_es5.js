...
$(function () {
  ...
  if (medium_zoom == 'false') {
    ...
    for (var i = 0; i < imgList.length; i++) {
      ...
      var $a = $('<a href="' + lazyload_src + '" data-fancybox="group" data-caption="' + imgList[i].alt + '" class="fancybox"></a>');
    ...
    zoom.on('open', function event() {
      var photoBg = $(document.documentElement).attr('data-theme') == 'dark' ? '#2d3032' : '#fff';
      zoom.update({
        background: photoBg
      });
    });
  ...
  $(window).scroll(throttle(function (event) {
    ...
    if (!isMobile()) {
      ...
      scrollPercent(currentTop); // head position
  ...
  if ($('.justified-gallery').length) {
    $('.justified-gallery img').each(function (i, o) {
